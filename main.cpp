...
int *GetIPAddress(char *ifname, char *ip_addr)
{
	int fd;
	struct ifreq ifr;
	...
	memcpy(ip_addr,inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr),16);
	close(fd);
...
int main(int argc, char* argv[]) {
	...
	//char my_ip_addr[4];
	//GetIPAddress(argv[1],my_ip_addr);
...
*/	req_packet.eth_.type_ = htons(EthHdr::Arp);
	...
	req_packet.arp_.sip_ = htonl(Ip(argv[3]));
	req_packet.arp_.tmac_ = Mac("00:00:00:00:00:00");
	req_packet.arp_.tip_ = htonl(Ip(argv[2]));
	...
	while (true){
		...
		EthArpPacket spf_packet;
        	...
        	spf_packet.arp_.sip_ = htonl(Ip(argv[3]));
        	spf_packet.arp_.tmac_ = rep_packet->arp_.smac_ ;
        	spf_packet.arp_.tip_ = htonl(Ip(argv[2]));
